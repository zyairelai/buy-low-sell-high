try:
    ...
    def record_tmp(core):
        ...
        with open((os.path.join("tmp", config.pair + ".txt")), "w", encoding="utf-8") as timestamp_record:
            timestamp_record.write(str(core))

    def retrieve_tmp():
        with open((os.path.join("tmp", config.pair + ".txt")), "r", encoding="utf-8") as timestamp_record:
            return float(timestamp_record.read())

    record_tmp(config.core) # Record the core number before running
    def buy_low_sell_high():
        my_core_number  = retrieve_tmp()
        ...
        trade_amount    = round(abs(current_core - my_core_number), config.round_off)

        if (current_core > my_core_number) and (abs(change_percent) > config.margin_percentage):
            ...
            print(colored("Prefix Core          : " + str(my_core_number) + " " + config.base, "green"))
        ...
        elif (current_core < my_core_number) and (abs(change_percent) > config.margin_percentage):
            if config.live_trade:
                client.order_market_buy(symbol=config.pair, quoteOrderQty=trade_amount)
            record_tmp(my_core_number + (config.core * 0.02))
            print(colored(asset_info, "red"))
            print(colored("Created at           : " + str(datetime.today().strftime("%d-%m-%Y @ %H:%M:%S")), "red"))
            print(colored("Prefix Core          : " + str(my_core_number) + " " + config.base, "red"))
            ...
            print("Prefix Core          : " + str(my_core_number) + " " + config.base)
